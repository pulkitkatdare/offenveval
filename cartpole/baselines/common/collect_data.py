...
def collect_data_for_IS(environment, episodes=1000, buffer_size=1000000, epsilon=0.3):
    D_P = {}
    steps = 0
...
def save_model_for_IS(data_buffer, environment='GridWorld10x10', key='p'):
    save_file = './cartpole/baselines/D_' + key + '.pkl'
    cPickle.dump(data_buffer, open(save_file, 'wb'))

def load_model_for_IS(environment='GridWorld10x10', key = ''):
    save_file = 'cartpole/baselines/D_' + key + '.pkl'
    print (save_file)
