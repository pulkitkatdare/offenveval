...
def main(estimate_beta = False, EPSILON=0.0):
    Batches =  [int(10**(5.5))]#[1000, 10000, 20000, 50000, 100000]#, 50000, 100000]
    algorithm = {}
    algorithm['true'] = {}
    algorithm['OEE'] = {}
    algorithm['oracle'] = {}
    ...
    '''
    with open('./GridWorld/environmentdata/GridWorldOracle_' + str(int(10*EPSILON)) +'.pkl', 'wb') as f:
...
def plot_eval(mean_output, std_output, ax, color):
    ...
    '''
    Batches =  [1000, int(10**(3.5)) , int(10**(4)), int(10**(4.5)), int(10**(5)), int(10**(5.5))]#[1000, 10000, 20000, 50000, 100000]#, 50000, 100000]
    data = {}
    data['oracle'] = {}
    data['algorithm'] = {}
    data['algorithm_oracle'] = {}
    
    data['oracle']['average'] = []
    data['oracle']['std'] = []
    

    data['algorithm']['average'] = []
    data['algorithm']['std'] = []
    
    data['algorithm_oracle']['average'] = []
    data['algorithm_oracle']['std'] = []
    for batch_size in Batches:

        data['oracle']['average'].append(statistics.mean(oracle['total_rewards'][batch_size]))
        data['oracle']['std'].append(statistics.stdev(oracle['total_rewards'][batch_size])/np.sqrt(LENGTH))
        
        data['algorithm']['average'].append(statistics.mean(algorithm['total_rewards'][batch_size]))
        data['algorithm']['std'].append(statistics.stdev(algorithm['total_rewards'][batch_size])/ np.sqrt(LENGTH))
        
        data['algorithm_oracle']['average'].append(statistics.mean(algorithm_oracle['total_rewards'][batch_size]))
        data['algorithm_oracle']['std'].append(statistics.stdev(algorithm_oracle['total_rewards'][batch_size])/np.sqrt(LENGTH))
        
        
    print (data['oracle']['average'])
    fig, ax = plt.subplots()
    X = np.arange(len(Batches))
    ax.plot(X, data['oracle']['average'], '--', color='#8B0000', linewidth=4)
    ax.plot(X, data['algorithm']['average'], marker='o', markerfacecolor='#00441b', markersize=7, color='#014636',linewidth=2)
    ax.plot(X, data['algorithm_oracle']['average'], '--', color='#084081', linewidth=4)
    mean_reward_oracle = np.asarray(data['oracle']['average'])
    std_reward_oracle = np.asarray(data['oracle']['std'])
    mean_reward_algorithm = np.asarray(data['algorithm']['average'])
    std_reward_algorithm = np.asarray(data['algorithm']['std'])
    mean_reward_algorithm_oracle = np.asarray(data['algorithm_oracle']['average'])
    print (mean_reward_algorithm_oracle)
    std_reward_algorithm_oracle = np.asarray(data['algorithm_oracle']['std'])

    ax.fill_between(X, mean_reward_oracle - 1 * std_reward_oracle, mean_reward_oracle + 1 * std_reward_oracle, color='#8B0000', alpha=0.1)
    ax.fill_between(X, mean_reward_algorithm - 1 * std_reward_algorithm, mean_reward_algorithm + 1 * std_reward_algorithm, color='#00441b', alpha=0.1)
    ax.fill_between(X, mean_reward_algorithm_oracle - 1 * std_reward_algorithm_oracle,
                    mean_reward_algorithm_oracle + 1 * std_reward_algorithm_oracle, color='#084081', alpha=0.1)

    plt.xticks(X, Batches)
    plt.legend(['Oracle', 'Importance Sampling (Ours)', 'Oracle Importance Sampling'])
    ax.set_xlabel('Sample Size', fontsize=16)
    ax.set_ylabel('Average Returns', fontsize=16)
    
    plt.grid(True)
    tikzplotlib.save("gridworld_evaluation.tex")
    plt.savefig('gridworld_eval_trained.png')
    '''

if __name__ == '__main__':

    fig, ax = plt.subplots()
    batch_size = int(10**(3.5))
    colors = ['#000000', '#00441b', '#7f0000', '#084081', '#4d004b']
    data_is = pickle.load(open('./GridWorld/Baselines/is_data.pkl','rb'))
    for index, epsilon in enumerate([0.0, 0.1, 0.3, 0.5, 0.7, 0.9]):

        algorithm = main(estimate_beta=False, EPSILON=epsilon)
        algorithm = pickle.load(open('./GridWorld/environmentdata/GridWorld_' + str(int(10*epsilon)) +'.pkl', 'rb'))
        mean_output, std_output = eval_data(algorithm, key='true')
        mean_output = (1-GAMMA)*mean_output 
        std_output = (1-GAMMA)*std_output/10.0
        plot_eval(mean_output, std_output, ax, colors[0])

        algorithm_oracle = pickle.load(open('./GridWorld/environmentdata/GridWorldOracle_' + str(int(10*epsilon)) +'.pkl', 'rb'))

        mean_output, std_output = eval_data(algorithm_oracle, key='oracle')
        mean_output = (1-GAMMA)*mean_output 
        std_output = (1-GAMMA)*std_output/10.0
        print (std_output)
        ...
        tikzplotlib.save('gridworld_evaluation' + str(int(10*epsilon)) + '_' + str(batch_size)+ '.tex')
        plt.savefig('gridworld_eval_trained_' + str(int(10*epsilon)) + '_' + str(batch_size)+'.png')
        #plt.show()
        fig, ax = plt.subplots()
plt.close()
