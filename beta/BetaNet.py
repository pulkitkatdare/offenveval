...
class BetaNetwork(nn.Module):
    ...
    def __init__(self, state_dim, action_bound, learning_rate, tau, seed, action_dim = 1):
        ...
        torch.manual_seed(self.seed)
        self.layer4 = nn.Linear(256, self.action_dim)
        torch.manual_seed(self.seed)
    ...
    def forward(self, x):
        ...
        scaled_y = y*self.action_bound + self.action_bound + 1e-10 #1e-5#torch.clamp(y, min=1e-5, max=self.action_bound) * 1.0#self.action_bound
    ...
    def train_step(self, states_p, states_q):
        ...
        for param in self.parameters():
            output += self.tau*torch.norm(param)**2
        output.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
